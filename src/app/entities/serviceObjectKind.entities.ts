...
export class ServiceObjectKind extends Item {
    @select()
    public ID: number;
    @select()
    public Title: string;
    ...
    public get(parser?: ODataParser<any>, getOptions?: FetchOptions): Promise<any> {
        ...
        if (parser === undefined) {
            parser = ODataEntity(ServiceObjectKind);
        }
    ...
    public getAs<T>(parser?: ODataParser<any>, getOptions?: FetchOptions): Promise<T> {
        this
            ...
            ._setCustomQueryFromDecorator("expand");
        if (parser === undefined) {
            parser = new SelectDecoratorsParser<ServiceObjectKind>(ServiceObjectKind);
        }
        return super.get.call(this, parser, getOptions);
    }

    private _setCustomQueryFromDecorator(parameter: string): ServiceObjectKind {
...
export class ServiceObjectKindList extends Items {

    private ItemTemplate: ServiceObjectKind = new ServiceObjectKind("");
    ...
    public get(parser?: ODataParser<any>, getOptions?: FetchOptions): Promise<any> {
        ...
        if (parser === undefined) {
            // default parser
            parser = ODataEntityArray(ServiceObjectKind);
        }
        return super.get.call(this, parser, getOptions);
    }

    // create new method using custom parser
    public getAsMyDocument(parser?: ODataParser<ServiceObjectKind[]>, getOptions?: FetchOptions): Promise<ServiceObjectKind[]> {
        this
            ._setCustomQueryFromDecorator("select")
            ._setCustomQueryFromDecorator("expand");
        if (parser === undefined) {
            parser = new SelectDecoratorsArrayParser<ServiceObjectKind>(ServiceObjectKind);
        }
        return super.get.call(this, parser, getOptions);
    }


    private _setCustomQueryFromDecorator(parameter: string): ServiceObjectKindList {
        const sym: string = getSymbol(parameter);
        // get pre-saved select and expand props from decorators
