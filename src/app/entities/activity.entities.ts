...
export class Activity extends Item {
    ...
    public getAs<T>(parser?: ODataParser<any>, getOptions?: FetchOptions): Promise<T> {
        this
            ...
            ._setCustomQueryFromDecorator("expand");
        if (parser === undefined) {
            parser = new SelectDecoratorsParser<Activity>(Activity);
        }
        return super.get.call(this, parser, getOptions);
    }

    private _setCustomQueryFromDecorator(parameter: string): Activity {
...
export class ActivityList extends Items {

    private ItemTemplate: Activity = new Activity("");
    ...
    public get(parser?: ODataParser<any>, getOptions?: FetchOptions): Promise<any> {
        ...
        if (parser === undefined) {
            // default parser
            parser = ODataEntityArray(Activity);
        }
        return super.get.call(this, parser, getOptions);
    }

    // create new method using custom parser
    public getAsMyDocument(parser?: ODataParser<Activity[]>, getOptions?: FetchOptions): Promise<Activity[]> {
        this
            ._setCustomQueryFromDecorator("select")
            ._setCustomQueryFromDecorator("expand");
        if (parser === undefined) {
            parser = new SelectDecoratorsArrayParser<Activity>(Activity);
        }
        return super.get.call(this, parser, getOptions);
    }


    private _setCustomQueryFromDecorator(parameter: string): ActivityList {
        const sym: string = getSymbol(parameter);
        // get pre-saved select and expand props from decorators
