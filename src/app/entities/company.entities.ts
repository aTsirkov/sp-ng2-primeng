...
export class Company extends Item {
    ...
    public getAs<T>(parser?: ODataParser<any>, getOptions?: FetchOptions): Promise<T> {
        this
            ...
            ._setCustomQueryFromDecorator("expand");
        if (parser === undefined) {
            parser = new SelectDecoratorsParser<Company>(Company);
        }
        return super.get.call(this, parser, getOptions);
    }

    private _setCustomQueryFromDecorator(parameter: string): Company {
...
export class CompanyList extends Items {

    private ItemTemplate: Company = new Company("");
    ...
    public get(parser?: ODataParser<any>, getOptions?: FetchOptions): Promise<any> {
        ...
        if (parser === undefined) {
            // default parser
            parser = ODataEntityArray(Company);
        }
        return super.get.call(this, parser, getOptions);
    }

    // create new method using custom parser
    public getAsMyDocument(parser?: ODataParser<Company[]>, getOptions?: FetchOptions): Promise<Company[]> {
        this
            ._setCustomQueryFromDecorator("select")
            ._setCustomQueryFromDecorator("expand");
        if (parser === undefined) {
            parser = new SelectDecoratorsArrayParser<Company>(Company);
        }
        return super.get.call(this, parser, getOptions);
    }


    private _setCustomQueryFromDecorator(parameter: string): CompanyList {
        const sym: string = getSymbol(parameter);
        // get pre-saved select and expand props from decorators
