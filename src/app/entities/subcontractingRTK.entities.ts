...
export class SubcontractingRTK extends Item {
    @select()
    public ID: number;
    @select()
    public Title: string;
    ...
    public get(parser?: ODataParser<any>, getOptions?: FetchOptions): Promise<any> {
        ...
        if (parser === undefined) {
            parser = ODataEntity(SubcontractingRTK);
        }
    ...
    public getAs<T>(parser?: ODataParser<any>, getOptions?: FetchOptions): Promise<T> {
        this
            ...
            ._setCustomQueryFromDecorator("expand");
        if (parser === undefined) {
            parser = new SelectDecoratorsParser<SubcontractingRTK>(SubcontractingRTK);
        }
        return super.get.call(this, parser, getOptions);
    }

    private _setCustomQueryFromDecorator(parameter: string): SubcontractingRTK {
...
export class SubcontractingRTKList extends Items {

    private ItemTemplate: SubcontractingRTK = new SubcontractingRTK("");
    ...
    public get(parser?: ODataParser<any>, getOptions?: FetchOptions): Promise<any> {
        ...
        if (parser === undefined) {
            // default parser
            parser = ODataEntityArray(SubcontractingRTK);
        }
        return super.get.call(this, parser, getOptions);
    }

    // create new method using custom parser
    public getAsMyDocument(parser?: ODataParser<SubcontractingRTK[]>, getOptions?: FetchOptions): Promise<SubcontractingRTK[]> {
        this
            ._setCustomQueryFromDecorator("select")
            ._setCustomQueryFromDecorator("expand");
        if (parser === undefined) {
            parser = new SelectDecoratorsArrayParser<SubcontractingRTK>(SubcontractingRTK);
        }
        return super.get.call(this, parser, getOptions);
    }


    private _setCustomQueryFromDecorator(parameter: string): SubcontractingRTKList {
        const sym: string = getSymbol(parameter);
        // get pre-saved select and expand props from decorators
